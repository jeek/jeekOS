...
export class Checkout {
    ...
    async installLoop() {
        ...
        while ((await Do(this.ns, "ns.singularity.getOwnedAugmentations", false))!.length + (12 - Math.ceil(((Date.now() - ((await Do(this.ns, "ns.getResetInfo")).lastAugReset))/60/60/1000))) >= ((await Do(this.ns, "ns.singularity.getOwnedAugmentations", true))!.length) && ((await Do(this.ns, "ns.singularity.getCurrentWork"))! == null || ((await Do(this.ns, "ns.singularity.getCurrentWork"))!.type != "FACTION" || (((await Do(this.ns, "ns.singularity.getCurrentWork"))!.type == "FACTION" && ((await Do(this.ns, "ns.singularity.getFactionFavor", (await Do(this.ns, "ns.singularity.getCurrentWork"))!.factionName))) < favorToDonate && ((await Do(this.ns, "ns.singularity.getFactionFavor", (await Do(this.ns, "ns.singularity.getCurrentWork"))!.factionName))) + ((await Do(this.ns, "ns.singularity.getFactionFavorGain", (await Do(this.ns, "ns.singularity.getCurrentWork"))!.factionName))) < favorToDonate))))) {
    ...
    async buyLoop() {
        while (true) {
            ...
            if (element >= 0) {
                ...
                let aug = record[2];
                while ((await Do(this.ns, "ns.getPlayer"))!.factions.includes(faction) && (await Do(this.ns, "ns.getPlayer"))!.money > 1e9 && (await Do(this.ns, "ns.singularity.getFactionFavor", faction))! >= 150 && (await Do(this.ns, "ns.singularity.getFactionRep", faction))! < (await Do(this.ns, "ns.singularity.getAugmentationRepReq", aug))!) {
                     await Do(this.ns, "ns.singularity.donateToFaction", faction, 1e9);
                }
